...
convertForm.addEventListener("submit", (e) => {
  ...
  let cRadio = document.getElementById("to-c");
      // create radio buttons for farenheit conversion accessed by ID 
...
function parseNeededData(jsonData, createPrevSearch) {
  ...
  let chanceOfSunshine = jsonData.weather[0].hourly[0].chanceofsunshine;
    ...
    // call the function with the parameters we created that will update with the users specific location search --------------- ACTION TO UPDATE THOSE PARAMTERS WITH USER SEARCH 
  updateHTML({
    feelsLike,
    area,
    region,
...
// Update the HTML with the data passed in the params
function updateHTML({
  feelsLike,
  area,
  region,
